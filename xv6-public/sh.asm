...
main(void)
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
      ...
      33:	e8 ab 0d 00 00       	call   de3 <open>
...
fork1(void)
{
  ...
  exit();
      ...
      if(chdir(buf+3) < 0)
      bd:	e8 51 0d 00 00       	call   e13 <chdir>
      ...
      e3:	50                   	push   %eax
      e4:	e8 e2 0c 00 00       	call   dcb <close>
      break;
...
00000180 <runcmd>:
{
  ...
  if(pid == -1)
    ...
    close(p[0]);
     ...
     1ee:	e8 d8 0b 00 00       	call   dcb <close>
    close(p[1]);
     1f3:	58                   	pop    %eax
     1f4:	ff 75 f4             	pushl  -0xc(%ebp)
     1f7:	e8 cf 0b 00 00       	call   dcb <close>
    wait();
  ...
  if(pid == -1)
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     235:	52                   	push   %edx
     236:	50                   	push   %eax
     237:	e8 9f 0b 00 00       	call   ddb <exec>
  ...
  if(pid == -1)
    ...
    close(rcmd->fd);
     ...
     27b:	e8 4b 0b 00 00       	call   dcb <close>
    if(open(rcmd->file, rcmd->mode) < 0){
     280:	58                   	pop    %eax
     281:	5a                   	pop    %edx
     282:	ff 73 10             	pushl  0x10(%ebx)
     285:	ff 73 08             	pushl  0x8(%ebx)
     288:	e8 56 0b 00 00       	call   de3 <open>
    ...
    panic("runcmd");
     ...
     2d1:	6a 01                	push   $0x1
     2d3:	e8 f3 0a 00 00       	call   dcb <close>
      dup(p[1]);
     2d8:	58                   	pop    %eax
     2d9:	ff 75 f4             	pushl  -0xc(%ebp)
     2dc:	e8 3a 0b 00 00       	call   e1b <dup>
      close(p[0]);
     2e1:	58                   	pop    %eax
     2e2:	ff 75 f0             	pushl  -0x10(%ebp)
     2e5:	e8 e1 0a 00 00       	call   dcb <close>
      close(p[1]);
     2ea:	58                   	pop    %eax
     2eb:	ff 75 f4             	pushl  -0xc(%ebp)
     2ee:	e8 d8 0a 00 00       	call   dcb <close>
      runcmd(pcmd->left);
     2f3:	5a                   	pop    %edx
     ...
     2ff:	6a 00                	push   $0x0
     301:	e8 c5 0a 00 00       	call   dcb <close>
      dup(p[0]);
     306:	5a                   	pop    %edx
     307:	ff 75 f0             	pushl  -0x10(%ebp)
     30a:	e8 0c 0b 00 00       	call   e1b <dup>
      close(p[0]);
     30f:	59                   	pop    %ecx
     310:	ff 75 f0             	pushl  -0x10(%ebp)
     313:	e8 b3 0a 00 00       	call   dcb <close>
      close(p[1]);
     318:	58                   	pop    %eax
     319:	ff 75 f4             	pushl  -0xc(%ebp)
     31c:	e8 aa 0a 00 00       	call   dcb <close>
      runcmd(pcmd->right);
     321:	58                   	pop    %eax
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     ce1:	e8 fd 00 00 00       	call   de3 <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     cf5:	50                   	push   %eax
     cf6:	e8 00 01 00 00       	call   dfb <fstat>
  close(fd);
     cfb:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
     cfe:	89 c6                	mov    %eax,%esi
  close(fd);
     d00:	e8 c6 00 00 00       	call   dcb <close>
  return r;
...
SYSCALL(resetread) //My change
     dbb:	b8 17 00 00 00       	mov    $0x17,%eax
     dc0:	cd 40                	int    $0x40
     dc2:	c3                   	ret    

00000dc3 <write>:
SYSCALL(write)
     dc3:	b8 10 00 00 00       	mov    $0x10,%eax
     dc8:	cd 40                	int    $0x40
     dca:	c3                   	ret    

00000dcb <close>:
SYSCALL(close)
     dcb:	b8 15 00 00 00       	mov    $0x15,%eax
     dd0:	cd 40                	int    $0x40
     dd2:	c3                   	ret    

00000dd3 <kill>:
SYSCALL(kill)
     dd3:	b8 06 00 00 00       	mov    $0x6,%eax
     dd8:	cd 40                	int    $0x40
     dda:	c3                   	ret    

00000ddb <exec>:
SYSCALL(exec)
     ddb:	b8 07 00 00 00       	mov    $0x7,%eax
     de0:	cd 40                	int    $0x40
     de2:	c3                   	ret    

00000de3 <open>:
SYSCALL(open)
     de3:	b8 0f 00 00 00       	mov    $0xf,%eax
     de8:	cd 40                	int    $0x40
     dea:	c3                   	ret    

00000deb <mknod>:
SYSCALL(mknod)
     deb:	b8 11 00 00 00       	mov    $0x11,%eax
     df0:	cd 40                	int    $0x40
     df2:	c3                   	ret    

00000df3 <unlink>:
SYSCALL(unlink)
     df3:	b8 12 00 00 00       	mov    $0x12,%eax
     df8:	cd 40                	int    $0x40
     dfa:	c3                   	ret    

00000dfb <fstat>:
SYSCALL(fstat)
     dfb:	b8 08 00 00 00       	mov    $0x8,%eax
     e00:	cd 40                	int    $0x40
     e02:	c3                   	ret    

00000e03 <link>:
SYSCALL(link)
     e03:	b8 13 00 00 00       	mov    $0x13,%eax
     e08:	cd 40                	int    $0x40
     e0a:	c3                   	ret    

00000e0b <mkdir>:
SYSCALL(mkdir)
     e0b:	b8 14 00 00 00       	mov    $0x14,%eax
     e10:	cd 40                	int    $0x40
     e12:	c3                   	ret    

00000e13 <chdir>:
SYSCALL(chdir)
     e13:	b8 09 00 00 00       	mov    $0x9,%eax
     e18:	cd 40                	int    $0x40
     e1a:	c3                   	ret    

00000e1b <dup>:
SYSCALL(dup)
     e1b:	b8 0a 00 00 00       	mov    $0xa,%eax
     e20:	cd 40                	int    $0x40
     e22:	c3                   	ret    

00000e23 <getpid>:
SYSCALL(getpid)
     e23:	b8 0b 00 00 00       	mov    $0xb,%eax
     e28:	cd 40                	int    $0x40
     e2a:	c3                   	ret    
     ...
     e5c:	89 d1                	mov    %edx,%ecx
{
  ...
  write(fd, &c, 1);
     ...
     ece:	56                   	push   %esi
     ecf:	57                   	push   %edi
     ed0:	e8 ee fe ff ff       	call   dc3 <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
  ...
  write(fd, &c, 1);
     ...
     f49:	e8 75 fe ff ff       	call   dc3 <write>
  ...
  write(fd, &c, 1);
     ...
     fa9:	e8 15 fe ff ff       	call   dc3 <write>
        putc(fd, c);
      } else {
  ...
  write(fd, &c, 1);
     ...
     fbd:	e8 01 fe ff ff       	call   dc3 <write>
  for(i = 0; fmt[i]; i++){
  ...
  write(fd, &c, 1);
    1034:	e8 8a fd ff ff       	call   dc3 <write>
        ap++;
  ...
  write(fd, &c, 1);
    ...
    10ab:	57                   	push   %edi
    10ac:	53                   	push   %ebx
    10ad:	e8 11 fd ff ff       	call   dc3 <write>
        while(*s != 0){
